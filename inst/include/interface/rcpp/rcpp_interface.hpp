...
void PrintOF(objective_function<TMB_FIMS_REAL_TYPE> *of)
{
  Rcpp::Rcout << "PrintOF" << std::endl;

  if (of != NULL)
  {
    if (of->report == NULL)
    {
      Rcpp::Rcout << "PrintOF report is NULL" << std::endl;
    }
    else
    {
      Rcpp::Rcout << "PrintOF report is not NULL" << std::endl;
    }

    Rcpp::Rcout << "PrintOF " << of->report << std::endl;
    Rcpp::Rcout << of->thetanames.size() << std::endl;
    for (int i = 0; i < of->thetanames.size(); i++)
    {
      Rcpp::Rcout << of->thetanames[i] << std::endl;
    }
    Rcpp::Rcout << of->parnames.size() << std::endl;
    for (int i = 0; i < of->parnames.size(); i++)
    {
      Rcpp::Rcout << of->parnames[i] << std::endl;
    }
  }
  else
  {
    Rcpp::Rcout << "OF is NULL" << std::endl;
  }
}

Rcpp::Environment GetReport()
{
  std::shared_ptr<fims_model::Model<TMB_FIMS_REAL_TYPE>> model =
      fims_model::Model<TMB_FIMS_REAL_TYPE>::GetInstance();
  // PrintOF(model->of);
  Rcpp::Rcout << "GetReport" << std::endl;
  if (model->of != NULL)
  {
    model->of->report = Rcpp::Environment();
    Rcpp::Rcout << "GetReport: " << model->of->report << std::endl;
    if(TYPEOF(model->of->report) == NILSXP)
    {
      Rcpp::Rcout << "GetReport: report is NULL" << std::endl;
    }
    else
    {
      Rcpp::Rcout << "GetReport: report is not NULL" << std::endl;
    }
    if(!Rf_isNull(model->of->report))Rf_error("'report' is null");
    SEXP env = R_ParentEnv(model->of->report);

    return env;// Rcpp::as<Rcpp::Environment>(model->of->report);
  }
...
RCPP_MODULE(fims)
{
  ...
  Rcpp::function(
      "GetReport", GetReport,
      "Returns the report object from the TMB model.");
}
