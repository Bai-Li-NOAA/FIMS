...
public:
  ...
  virtual double evaluate(double x) {
    ...
    LogisticMat.inflection_point[0] = this->inflection_point[0].value_m;
    LogisticMat.slope.resize(1);
    LogisticMat.slope[0] = this->slope[0].value_m;
  ...
  bool add_to_fims_tmb_internal() {
    ...
    maturity->id = this->id;
        ...
        for (size_t i = 0; i < this->inflection_point.size(); i++) {
            maturity->inflection_point[i] = this->inflection_point[i].value_m;
            if (this->inflection_point[i].estimated_m) {
                ss.str("");
        ...
        for (size_t i = 0; i < this->slope.size(); i++) {
            maturity->slope[i] = this->slope[i].value_m;
            if (this->slope[i].estimated_m) {
                ss.str("");
