    ...
    public:
        ...
        void SetupData()
        {
            for (density_components_iterator it = this->density_components.begin(); it != this->density_components.end(); ++it)
            {
                ...
                if (d->input_type == "data")
                {
                    ...
                    {
                        vmit = this->variable_map.find(d->key[i]);
                        FIMS_INFO_LOG("Link expected value from distribution " + fims::to_string(d->id) + " to derived value " + fims::to_string(d->key[i]));
            ...
            std::shared_ptr<fims_popdy::Fleet<Type>> f)
        {
            ...
            else
            {
                ...
                FIMS_ERROR_LOG("No index data observed for fleet " + fims::to_string(f->id) + ". FIMS requires index data for all fleets.");
            }
        }
