...
class BevertonHoltRecruitmentInterface : public RecruitmentInterfaceBase {
 public:
  ...
  virtual double evaluate(double spawners, double ssbzero) {
    ...
    BevHolt.logit_steep[0] = this->logit_steep[0].value_m;
    if (this->logit_steep[0].value_m == 1.0) {
      warning(
    ...
    BevHolt.log_rzero[0] = this->log_rzero[0].value_m;
        ...
        if (it == info->recruitment_models.end()) {
            ...
            for (size_t i = 0; i < this->logit_steep.size(); i++) {
                if (this->logit_steep[i].estimated_m) {
                    this->logit_steep[i].estimated_value_m = recr->logit_steep[i];
                } else {
                    this->logit_steep[i].estimated_value_m = this->logit_steep[i].value_m;
                }
            }

            for (size_t i = 0; i < log_rzero.size(); i++) {
                if (log_rzero[i].estimated_m) {
                    this->log_rzero[i].estimated_value_m = recr->log_rzero[i];
                } else {
                    this->log_rzero[i].estimated_value_m = this->log_rzero[i].value_m;
                }
            }
            ...
            for (size_t i = 0; i < this->estimated_log_devs.size(); i++) {
                if (this->log_devs[i].estimated_m) {
                    ...
                    this->log_devs[i].estimated_value_m = this->log_devs[i].value_m;
                }
            }
        }
  ...
  bool add_to_fims_tmb_internal() {
    ...
    //set logit_steep
        ...
        for (size_t i = 0; i < this->logit_steep.size(); i++) {
            ...
            recruitment->logit_steep[i] = this->logit_steep[i].value_m;
        ...
        for (size_t i = 0; i < this->log_rzero.size(); i++) {
            ...
            recruitment->log_rzero[i] = this->log_rzero[i].value_m;
